...
MONO_D3D12_API void mono_d3d12::ConvertToHRtoStr(HRESULT hr, std::string &hrStr)
{
    char* msgBuf = nullptr;
    ...
    hrStr = (size && msgBuf) ? msgBuf : "Unknown error";
    if (msgBuf) LocalFree(msgBuf);
}

MONO_D3D12_API void mono_d3d12::CreateDX12Factory(ComPtr<IDXGIFactory4> &factory)
{
    ...
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create DXGI Factory: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateDX12Device
(
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Device: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateDX12CommandQueue
(
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Command Queue: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::SetName(const ComPtr<ID3D12Resource> &object, const std::wstring &name)
{
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to set name for D3D12 object: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateSwapChain
(
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create DXGI Swap Chain: " + hrStr}, RIAECS_LOG_LOC);
    }
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to cast IDXGISwapChain1 to IDXGISwapChain3: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::ResizeSwapChain
(
    const UINT &frameCount, UINT &frameIndex, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to resize DXGI Swap Chain: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::GetBuffersFromSwapChain
(
    ...
    for (UINT i = 0; i < frameCount; ++i)
    {
        ...
        if (FAILED(hr))
        {
            std::string hrStr;
            mono_d3d12::ConvertToHRtoStr(hr, hrStr);
            riaecs::NotifyError({"Failed to get buffer from DXGI Swap Chain: " + hrStr}, RIAECS_LOG_LOC);
        }
...
MONO_D3D12_API void mono_d3d12::CreateRenderTargetViewHeap
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Render Target View Heap: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::CreateRenderTargetView
(
    const ComPtr<ID3D12Device4> &device, 
...
MONO_D3D12_API void mono_d3d12::CreateCommandAllocator
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Command Allocator: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateDepthStencil
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Depth Stencil: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateDepthStencilViewHeap
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Depth Stencil View Heap: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::CreateDepthStencilView
(
    const ComPtr<ID3D12Device4> &device,
...
MONO_D3D12_API void mono_d3d12::CreateViewport(D3D12_VIEWPORT &viewport, const UINT &clientWidth, const UINT &clientHeight)
{
...
MONO_D3D12_API void mono_d3d12::CreateScissorRect(D3D12_RECT &scissorRect, const UINT &clientWidth, const UINT &clientHeight)
{
    scissorRect.top = 0;
...
MONO_D3D12_API void mono_d3d12::CreateBuffer
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Buffer: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::UpdateBuffer
(
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to map D3D12 Buffer: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::CreateTexture2D
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Texture2D: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateUploadHeap
(
    const ComPtr<ID3D12Device4> &device, 
...
MONO_D3D12_API UINT mono_d3d12::GetDXGIFormatByteSize(const DXGI_FORMAT &format)
{
    switch (format)
    {
    ...
    default:
        riaecs::NotifyError({"Unsupported DXGI format: " + std::to_string(static_cast<int>(format))}, RIAECS_LOG_LOC);
    }

    return 0; // should not be reached
}

MONO_D3D12_API void mono_d3d12::AddUploadTextureToCmdList
(
    ...
    subresourceData.RowPitch = (width * mono_d3d12::GetDXGIFormatByteSize(format) + 255) & ~255; // Align to 256 bytes
...
MONO_D3D12_API void mono_d3d12::CreateTexture2DSRV
(
    const ComPtr<ID3D12Device4> &device, 
...
MONO_D3D12_API void mono_d3d12::CreateCommandList
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Command List: " + hrStr}, RIAECS_LOG_LOC);
    }

    CloseCommand(commandList);
}

MONO_D3D12_API void mono_d3d12::CreateFence
(
    const ComPtr<ID3D12Device4> &device, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to create D3D12 Fence: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::CreateFenceEvent(HANDLE &fenceEvent)
{
    fenceEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    if (fenceEvent == nullptr)
        riaecs::NotifyError({"Failed to create fence event: " + std::to_string(GetLastError())}, RIAECS_LOG_LOC);
}

MONO_D3D12_API void mono_d3d12::CloseFenceEvent(HANDLE &fenceEvent)
{
    if (fenceEvent != nullptr)
    {
        bool result = CloseHandle(fenceEvent);
        if (!result)
            riaecs::NotifyError({"Failed to close fence event: " + std::to_string(GetLastError())}, RIAECS_LOG_LOC);

        fenceEvent = nullptr;
    }
}

MONO_D3D12_API void mono_d3d12::CloseCommand(const ComPtr<ID3D12GraphicsCommandList> &commandList)
{
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to close D3D12 Command List: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::ResetCommand
(
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to reset D3D12 Command Allocator: " + hrStr}, RIAECS_LOG_LOC);
    }
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to reset D3D12 Command List: " + hrStr}, RIAECS_LOG_LOC);
    }
}

MONO_D3D12_API void mono_d3d12::ExecuteCommand
(
    const ComPtr<ID3D12CommandQueue> &commandQueue, 
    const UINT &commandListCount, std::vector<ID3D12CommandList *> &commandLists
){
    commandQueue->ExecuteCommandLists(commandListCount, commandLists.data());
}

MONO_D3D12_API void mono_d3d12::WaitForGPU
(
    const ComPtr<ID3D12CommandQueue> &commandQueue, 
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to signal command queue: " + hrStr}, RIAECS_LOG_LOC);
    }
    ...
    if (FAILED(hr))
    {
        std::string hrStr;
        mono_d3d12::ConvertToHRtoStr(hr, hrStr);
        riaecs::NotifyError({"Failed to set event on fence completion: " + hrStr}, RIAECS_LOG_LOC);
    }
...
MONO_D3D12_API void mono_d3d12::WaitFrameForGPU
(
    const ComPtr<ID3D12CommandQueue> &commandQueue, 
...
MONO_D3D12_API void mono_d3d12::SetBarrier
(
    const ComPtr<ID3D12GraphicsCommandList> &cmdList, 
...
MONO_D3D12_API void mono_d3d12::SetRenderTargets
(
    const ComPtr<ID3D12GraphicsCommandList> &cmdList, 
    ...
    if (!rtvHeap)
        riaecs::NotifyError({"Render Target View Heap is null."}, RIAECS_LOG_LOC);

    if (!dsvHeap)
        riaecs::NotifyError({"Depth Stencil View Heap is null."}, RIAECS_LOG_LOC);
...
MONO_D3D12_API void mono_d3d12::ClearRenderTargetViews
(
    const ComPtr<ID3D12GraphicsCommandList> &cmdList, 
    const ComPtr<ID3D12DescriptorHeap> &rtvHeap, const UINT &rtvIndex, const UINT &rtvDescriptorSize, 
...
MONO_D3D12_API void mono_d3d12::ClearDepthStencilViews
(
    const ComPtr<ID3D12GraphicsCommandList> &cmdList, 
    const ComPtr<ID3D12DescriptorHeap> &dsvHeap, const UINT &dsvIndex, const UINT &dsvDescriptorSize, 
